...
from utils.dataloaders import LoadImages, CalibratorImages
...
import numpy as np
from torch2trt.calibration import (
    TensorBatchDataset,
    DatasetCalibrator,
    ...
    try:
        ...
        if simplify:
            try:
                ...
                #LOGGER.info(f"{prefix} Running ONNXRuntime Optimizations")
                #sess_options = rt.SessionOptions()
                #sess_options.graph_optimization_level = rt.GraphOptimizationLevel.ORT_ENABLE_ALL
                #sess_options.optimized_model_filepath = str(f)
                #session = rt.InferenceSession(str(f), sess_options)
                #LOGGER.info(f"{prefix} ONNXRuntime Optimizations complete")
            except Exception as e:
...
def export_engine(model, 
    ...
    try:
        ...
        if int8:
            ...
            # Enabling fallback to FP16 in case INT8 is slower
            config.set_flag(trt.BuilderFlag.FP16)
            ...
            if calib_algo == "entropy2":
                ...
                calib_algorithm = trt.CalibrationAlgoType.ENTROPY_CALIBRATION
            else:
                calib_algorithm = trt.CalibrationAlgoType.MINMAX_CALIBRATION
             
            ############### API changed in torch2trt v0.4.0 #################
            ################ code below not a bug #################
            calibrator = DatasetCalibrator(
                inputs=im, dataset=int8_calib_dataset, batch_size=calib_batch_size, algorithm=calib_algorithm
            )

            config.int8_calibrator = calibrator
            
        if sparsify:
            LOGGER.info(f'{prefix} applying sparsity optimizations')
            config.set_flag(trt.BuilderFlag.SPARSE_WEIGHTS)
        
        with builder.build_engine(network, config) as engine, open(f, 'wb') as t:
            t.write(engine.serialize())
        LOGGER.info(f'{prefix} export success, saved as {f} ({file_size(f):.1f} MB)')
        os.remove(file.with_suffix('.onnx'))
        return f
    except Exception as e:
        LOGGER.info(f'\n{prefix} export failure: {e}')
...
def run(
        ...
        int8=False,  # CoreML/TF/TensorRT INT8 quantization
        sparsify=False, # sparsify model
        prop=0.3, # proportion of sparsification
        struct=False, #enable structured sparsification
        calibrate=False, #TensorRT INT8 calibration
        calib_batch_size=4, #TensorRT INT8 calibration batch size
        calib_algo="entropy2", #INT8 calibration algorithm
        calib_num_images=10, #number of images to be used for calibration
        end2end=False, #include nms in tensorrt model
        seed=10, #seed for int8 calibration
    ...
    if sparsify:
        LOGGER.info(f"\n{colorstr('PyTorch:')} Sparsifying {file} by {prop*100}%")
        LOGGER.info(f"\n{colorstr('PyTorch:')} Pruning all weights")
        
        modules = [module[0] for module in list(model.named_parameters())]
        parameters_to_prune = []
        for module in modules:
            if 'weight' in module:
                obj = get_module_by_name(model, module)
                if struct and not isinstance(obj, nn.BatchNorm2d):
                    prune.ln_structured(obj, name='weight', amount=prop, n=1, dim=0)
        ...
        LOGGER.info(f"\n{colorstr('PyTorch:')} Sparsification successful")
    ...
    im = torch.ones(batch_size, 3, *imgsz).to(device)  # image size(1,3,320,192) BCHW iDetection
    ...
    if engine:  # TensorRT required before ONNX
        if int8 and calibrate:
            int8_calib_dataset = CalibratorImages('../datasets/coco/val2017/*.jpg', auto=False, num_images=calib_num_images, seed=seed)
        else:
            int8_calib_dataset = None

        f[1] = export_engine(model, 
                             im, 
                             file, 
                             train, 
                             half, 
...
def parse_opt():
    ...
    parser.add_argument('--int8', action='store_true', help='CoreML/TF/TensorRT INT8 quantization')
    parser.add_argument('--sparsify', action='store_true', default=False, help='sparsify the model (note: only weights are sparsified)')
    parser.add_argument('--prop', type=float, default=0.3, help='proportion of sparsification')
    parser.add_argument('--struct', default=False, action='store_true', help='enable structured sparsification')
    parser.add_argument('--calibrate', action='store_true', default=False, help='enable INT8 calibration')
    parser.add_argument('--calib-num-images', type=int, default=10, help='number of images to be used for calibration (max 1000)')
    parser.add_argument('--end2end', action='store_true', default=False, help='add NMS plugin to TensorRT model')
    parser.add_argument('--calib-batch-size', default=4, type=int, help='INT8 calibration batch size')
    parser.add_argument('--calib-algo', default='entropy2', choices=["entropy2", "entropy", "minmax"], help="INT8 calibration algorithm")
    parser.add_argument('--seed', default=10, type=int, help='seed for INT8 calibration')
    parser.add_argument('--dynamic', action='store_true', help='ONNX/TF: dynamic axes')
    parser.add_argument('--simplify', action='store_true', help='ONNX: simplify model')
    parser.add_argument('--opset', type=int, default=12, help='ONNX: opset version')
    parser.add_argument('--verbose', action='store_true', help='TensorRT: verbose log')
    parser.add_argument('--workspace', type=int, default=28, help='TensorRT: workspace size')
